...
from urllib.parse import urlencode as _pm_urlencode, urlparse as _pm_urlparse, parse_qs as _pm_parse_qs
...
def _pm_extract_ieee_arnumber(url: str) -> str:
    u = (url or "").strip()
    if not u:
        return ""
    try:
        pr = _pm_urlparse(u)
        q = _pm_parse_qs(pr.query or "")
        ar = (q.get("arnumber") or [""])[0].strip()
        if ar.isdigit():
            return ar
    except Exception:
        pass
    m = _pm_re.search(r"/document/(\d+)", u)
    if m:
        return m.group(1)
    m = _pm_re.search(r"/(\d+)\.pdf(?:$|\?)", u)
    if m:
        return m.group(1)
    return ""


def _pm_ieee_candidate_urls(url: str) -> list[str]:
    base = (url or "").strip()
    out = [base] if base else []
    ar = _pm_extract_ieee_arnumber(base)
    if ar:
        for u in [
            ...
            f"https://ieeexplore.ieee.org/ielx7/{ar}.pdf",
        ]:
            if u not in out:
                out.append(u)
...
async def _pm_try_pdf_text_from_bytes(data: bytes, max_pages: int, max_chars: int) -> tuple[bool, dict]:
    row = {"ok": False, "error": None, "pages_read": 0, "chars": 0, "text": ""}
    if not data:
        row["error"] = "empty_pdf_bytes"
        return False, row
    try:
        ...
        row["error"] = f"pdf_parse_error: {str(e)[:180]}"
        return False, row


async def _pm_fetch_ieee_via_playwright(url: str, max_pages: int, max_chars: int) -> dict:
    row = {"url": url, "norm_url": url, "used_url": "", "ok": False, "error": None,
           "pages_read": 0, "chars": 0, "text": "", "method": "playwright_ieee", "debug": {"attempts": []}}
    try:
        from playwright.async_api import async_playwright
    except Exception as e:
        row["error"] = f"playwright_missing: {e}"
        return row

    candidates = _pm_ieee_candidate_urls(url)
    try:
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True, args=["--no-sandbox", "--disable-dev-shm-usage", "--disable-gpu"])
            ctx = await browser.new_context(user_agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124 Safari/537.36")
            page = await ctx.new_page()
            discovered = []

            # 1) Visit doc pages to discover real PDF hrefs in same session
            for c in list(candidates):
                try:
                    resp = await page.goto(c, wait_until="domcontentloaded", timeout=45000)
                    row["debug"]["attempts"].append({"step":"goto","url":c,"status":(resp.status if resp else None),"ok":bool(resp)})
                    if resp is not None:
                        ctype = (resp.headers.get("content-type") or "").lower()
                        if "pdf" in ctype:
                            body = await resp.body()
                            ok, parsed = await _pm_try_pdf_text_from_bytes(body, max_pages, max_chars)
                            if ok:
                                row.update(parsed)
                                row["norm_url"] = c
                                row["used_url"] = c
                                row["method"] = "playwright_goto_pdf"
                    ...
                    for h in hrefs or []:
                        ...
                        if h.startswith('//'):
                            ...
                            h = 'https://ieeexplore.ieee.org' + h
                        if ('stampPDF/getPDF.jsp' in h or '/ielx' in h or h.lower().endswith('.pdf')) and h not in discovered:
                            discovered.append(h)
                except Exception:
                    continue

            # 2) Try request context with cookies/session
            all_pdf_urls = []
            for u in candidates + discovered:
                if u and u not in all_pdf_urls:
                    all_pdf_urls.append(u)

            for pu in all_pdf_urls:
                try:
                    rr = await ctx.request.get(pu, timeout=60000)
                    row["debug"]["attempts"].append({"step":"request","url":pu,"status":rr.status,"ctype":(rr.headers.get("content-type") or "")[:80]})
                    if rr.status >= 400:
                        continue
                    ...
                    if ok:
                        ...
                        row["norm_url"] = pu
                        row["used_url"] = pu
                        row["method"] = "playwright_request_pdf"
                        await page.close(); await ctx.close(); await browser.close()
                        return row
                except Exception:
                    continue
    ...
    row["error"] = "ieee_playwright_failed"
    return row
...
def _pm_normalize_pdf_url(url: str) -> str:
    ...
    if not u:
        return ""
    lu = u.lower()
    if "ieeexplore.ieee.org" in lu:
        ar = _pm_extract_ieee_arnumber(u)
        if ar:
...
async def _pm_fetch_pdf_excerpt(pdf_url: str, max_pages: int, max_chars: int) -> dict:
    ...
    row = {"url": pdf_url, "norm_url": norm, "used_url": "", "ok": False, "error": None,
           "pages_read": 0, "chars": 0, "text": "", "debug": {"attempts": []}}
    if not norm:
        row["error"] = "empty_or_unsupported_url"
        return row

    # IEEE часто режет прямой HTTP (418). Сначала пробуем браузерный путь.
    if "ieeexplore.ieee.org" in (pdf_url or "").lower() or "ieeexplore.ieee.org" in (norm or "").lower():
        pw = await _pm_fetch_ieee_via_playwright(pdf_url or norm, max_pages, max_chars)
        if pw.get("ok"):
            return pw
        row["debug"]["attempts"].append({"step":"playwright_ieee","ok":False,"error":pw.get("error")})

    try:
        async with httpx.AsyncClient(timeout=60.0, follow_redirects=True) as client:
            r = await client.get(norm, headers={"User-Agent": "Mozilla/5.0"})
            row["debug"]["attempts"].append({"step":"httpx_get","url":norm,"status":r.status_code,"ctype":(r.headers.get("content-type") or "")[:80]})
            r.raise_for_status()
            data = r.content
        ...
        if "pdf" not in ctype and bytes(data[:5]) != b"%PDF-":
            ...
            return row
        ok, parsed = await _pm_try_pdf_text_from_bytes(data, max_pages, max_chars)
        if ok:
            row.update(parsed)
            row["used_url"] = norm
            row["method"] = "httpx_direct_pdf"
        else:
